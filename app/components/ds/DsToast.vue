...
.ds-toast {
  ...
  /* Kính đặc — escape stacking context, đọc tốt trên mọi nền */
...
/* ── Variants — viền trái + icon màu theo token (hợp cả light/dark) ── */
.ds-toast--success { border-left-color: var(--ok); }
.ds-toast--success .ds-toast-ic  { color: var(--ok); }
.ds-toast--success .ds-toast-close:hover { background: color-mix(in oklab, var(--ok) 15%, transparent); }

.ds-toast--error { border-left-color: var(--danger); }
.ds-toast--error .ds-toast-ic  { color: var(--danger); }
.ds-toast--error .ds-toast-close:hover { background: color-mix(in oklab, var(--danger) 15%, transparent); }

.ds-toast--warning { border-left-color: var(--warn); }
.ds-toast--warning .ds-toast-ic  { color: var(--warn); }
.ds-toast--warning .ds-toast-close:hover { background: color-mix(in oklab, var(--warn) 15%, transparent); }

.ds-toast--info { border-left-color: var(--info); }
.ds-toast--info .ds-toast-ic  { color: var(--info); }
.ds-toast--info .ds-toast-close:hover { background: color-mix(in oklab, var(--info) 15%, transparent); }
